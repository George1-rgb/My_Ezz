...
namespace My_Ezz
{
		...
		: m_nWidth(nWidth), m_nHeight(nHeight)
	{
		...
		bool bUseRGBA = textureType == TextureType::kDiffuse || textureType == TextureType::kSkyBox;
		glTextureSubImage2D(m_nID, 0, 0, 0, m_nWidth, m_nHeight, bUseRGBA ? GL_RGBA : GL_RGB, GL_UNSIGNED_BYTE, cData);
		glTextureParameteri(m_nID, GL_TEXTURE_WRAP_S, GL_REPEAT);
		glTextureParameteri(m_nID, GL_TEXTURE_WRAP_T, GL_REPEAT);
		glTextureParameteri(m_nID, GL_TEXTURE_MIN_FILTER, textureType == TextureType::kSkyBox ? GL_NEAREST : GL_LINEAR_MIPMAP_NEAREST);
		glTextureParameteri(m_nID, GL_TEXTURE_MAG_FILTER, textureType == TextureType::kSkyBox ? GL_NEAREST : GL_LINEAR);
